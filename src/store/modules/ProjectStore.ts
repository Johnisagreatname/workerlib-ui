...
export default class ProjectStore extends VuexModule {
  ...
  public async getCompany(id): Promise<any> {
    request
      .post("/api/workerlib/company_hierarchy", {
        pageInfo: {},
        conditionList: [
          {
            name: "pid",
  ...
  public async insertUnitCompany() {
    ...
    request
      .put("/api/workerlib/company_hierarchy", {
        id: null,
  ...
  public async insertProject() {
    request
      .put("/api/workerlib/project", {
        ...
        start_time:
          this.insertProjectStartTime.getFullYear() +
          "-" +
          (this.insertProjectStartTime.getMonth() + 1) +
          "-" +
          this.insertProjectStartTime.getDate(),
        end_time:
          this.insertProjectEndTime.getFullYear() +
          "-" +
          (this.insertProjectEndTime.getMonth() + 1) +
          "-" +
          this.insertProjectEndTime.getDate(),
  ...
  public async uploadProjectCompany() {
    let alert: any = Message;
    await request
      .post("/api/workerlib/company_hierarchy/export", {
        conditionList: [
          {
            name: "pid",
  ...
  public async deleteProjectCompanyId() {
    await request
      .delete("/api/workerlib/company_hierarchy/" + this.deleteCompanyId)
      .then((data) => {
        if (!data) {
          return;
        }
