...
export default class LecturerStore extends VuexModule {
  ...
  public async searchLecturerList() {
    await request
      .post("/api/workerlib/lecturer", await this.getLecturerListParams())
      .then((data) => {
        if (!data) {
          return;
        }
  ...
  public async searchLecturerListCount() {
    await request
      .post("/api/workerlib/lecturer/count", await this.getLecturerListParams())
      .then((total) => {
        if (!total) {
          return;
        }
  ...
  public async updateLecturer() {
    await request
      .put("/api/workerlib/lecturer/" + this.updateLecturerId, {
  ...
  public async deleteLecturerById() {
    await request
      .delete("/api/workerlib/lecturer/" + this.deleteLecturerId)
      .then((data) => {
        if (!data) {
          return;
        }
