...
export default class CheckStore extends VuexModule {
  ...
  public async searchData({
    ...
    return new Promise(async (resolve, reject) => {
      let params =
        conName && (status || status === 0)
          ? [{ name: conName, value: status }]
          : null;
      await request
        .post("/api/workerlib/" + url, await this.getUpdateParams(params))
        .then((data) => {
          if (!data) {
            return;
          }
  ...
  public async searchEvaluate() {
    await request
      .post("/api/workerlib/skillappraisal", await this.getSearchParams())
      .then((total) => {
        if (!total) {
          return;
        }
  ...
  public async evaluateCount() {
    await request
      .post("/api/workerlib/skillappraisal/count", await this.getSearchParams())
      .then((total) => {
        if (!total) {
          return;
        }
